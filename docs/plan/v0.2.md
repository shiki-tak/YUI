...
**ターン全体の時間**：解釈の timeout（候補 10 秒）＋生成＋再生成1回。上限の候補は 45 秒。**PR5 で測った**：解釈を有効にすると1ターン平均 +77%〜+184%（測定条件で変動）増え、実測の平均は11.9〜12.0秒・最大12.7〜14.3秒（測定回で変動）で、45秒には十分な余裕がある。解釈は一度は待ち時間と判定精度を理由に既定で無効にしたが、**2026-09-16 に既定で有効へ変更した**（timeout も 30 秒）。[result/v0.2.md](../result/v0.2.md) の「既定を有効に変更した」。
...
| 名前 | 既定 | 場所 |
| --- | --- | --- |
| `conversation_state_llm` | **`True`**（PR5 では `False` と判断したが、その根拠3件が誤った原因帰属だったと分かり 2026-09-16 に覆した。[result/v0.2.md](../result/v0.2.md) の「既定を有効に変更した」） | `Settings` |
| `conversation_state_llm_timeout_seconds` | **`30.0`**（10.0 では単体実測 3.9〜9.2 秒に対し余裕が最小 0.8 秒で、同時実行下の timeout が多発した。30 秒で解釈 63/63 成功。[result/v0.2.md](../result/v0.2.md)） | `Settings` |
| `conversation_state_context_messages` | `6` | `Settings` |
| `PRESENTED_LIMIT` | `5`（窓の外の `presented` を渡す上限） | `conversation_state.py` の定数（`Settings` の項目ではない。第4回レビューで合意済みだが表側の訂正が漏れていた） |
| `conversation_state_confirm_offer_limit` | `2`（食い違いの確認候補を渡す回数の上限） | `Settings` |
| `turn_time_budget_seconds` | `45.0`（PR5 で実測。解釈ありの実測平均11.9秒（`172947`）〜12.0秒（`171509`）、最大12.7秒（`172947`）〜14.3秒（`171509`）に対し余裕あり。まだ何も強制しない） | `Settings` |
...
| **PR3 解釈の呼び出し** | スキーマの JSON、ID と操作可能性の検証、timeout、失敗時は規則へ。`request`・`confirmed`・`correction`・`discrepancy`、`answered`／`unanswered`／`withdrawals` による解決・取消・答え損ねの判定と、`judged` による再判定。`asked_discrepancy_ids` による確認の照合、`resolved_discrepancy_ids` による解決（訂正と同じトランザクション）、渡す回数の上限。相づち辞書の制約。シナリオ 5・7・9〜18・20〜24。当時は既定無効（2026-09-16 に有効へ変更） | 評価器で 5・7・9〜18・20〜24。**不適切な沈黙**と**過剰確認**の側。解釈のタイムアウトの pytest |
...
**順序の理由**：狭いケース（規則だけ）で状態管理と定型処理を先に通し（PR1・2）、意味の解釈が要るところにだけ LLM を足し（PR3）、API・画面を接続し（PR4）、利用者から見た振る舞いを統合評価する（PR5）。PR3 まで解釈は既定で無効だったので、PR1・2 の結果は解釈の揺れを含まない（既定は 2026-09-16 に有効へ変更した。当時の結果の読み方は変わらない）。PR2 の受け入れ条件は PR3 に依存しない。
