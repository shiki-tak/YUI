...
**ターン全体の時間**：解釈の timeout（候補 10 秒）＋生成＋再生成1回。上限の候補は 45 秒。**PR5 で測った**：解釈を有効にすると1ターン平均 +77%〜+184%（測定条件で変動）増え、実測の平均は11.9〜12.0秒・最大12.7〜14.3秒（測定回で変動）で、45秒には十分な余裕がある。解釈は待ち時間と実モデルの判定精度の両方を理由に既定で無効のまま（[result/v0.2.md](../result/v0.2.md)）。
...
| 名前 | 既定 | 場所 |
| --- | --- | --- |
| `conversation_state_llm` | `False`（PR5 で判断。[result/v0.2.md](../result/v0.2.md) に根拠） | `Settings` |
| `conversation_state_llm_timeout_seconds` | `10.0`（呼び出し単体の所要時間は測っておらず、単独実行で超えたかは直接確認できていない。間接的な見積もりでは超えた回もある。詳細は [result/v0.2.md](../result/v0.2.md)） | `Settings` |
