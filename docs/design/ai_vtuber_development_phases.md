...
- フェーズ4（経験に基づく自発性）は PR1〜PR12 として実装したが、実測で設計の問題が見えたため、v0.1 の定義には含めず **v1.0 として作り直す**。既存コードの残す・削るは実装計画の第1.6節に書いた。
...
本フェーズは PR1〜PR12 として **実装済み**です（[計画](../plan/phase4.md)、[記録](../result/phase4.md)、[現在地](../result/phase4_status.md)）。ただし実モデルの測定で、設計そのものの問題が2つ見えました。

- **LLM に「話すか待つか」を判断させると、待機に倒れる。** 指示文で禁じても効かない（[ISSUE-031](../issues/issues.md#issue-031)）。
- **目標の自動採用は、いまの抽出品質では成立しない。** 1つの会話から重複した目標が複数出る（[phase4_status.md](../result/phase4_status.md)）。

そのため、[人格の実装計画](artificial-personhood.md) で **v1.0 として作り直します**。中心となる変更は、発火するかを LLM ではなくコードで決めることです。既存コードの残す・削るは実装計画の第1.6節に書きました。
...
| 5 行動と状態変化の根拠を追え、悪化時に戻せる | v1.0 | `run_records`・`goal_revisions`・`asked_message_id`。`action_records` は削除 |
...
1. フェーズ4 実装のうち、v1.0 で不要なもの（LLM による行動選択、`answered` / `cancelled` の判定、`action_records`、聞き直し間隔）を削除する。再生状態による実行済み判定（`delivery.py`）は `asked_message_id` を足して依存を切り、残す。
