...
| 2：アバターと音声 | 文字で話しかけると、自分の画像のキャラクターが声で返す | PNG表示、音声合成、字幕・口パク |
...
図の元データは [dev_flow.mmd](dev_flow.mmd) にあります。更新手順はファイル冒頭のコメントに書いています。
...
用意した画像のキャラクターが、声と口パクで返答するようにします。入力は文字のままとし、音声入力は行いません。
...
5. 送信から音声の再生開始までの時間を測り、待ち時間の大きい部分を確認する。
...
最初は静止画でも進められます。口パク・まばたきには口と目の開閉差分を用意します。
