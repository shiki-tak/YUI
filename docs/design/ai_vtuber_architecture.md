...
- [v0.2 の詳細設計と PR 計画](../plan/v0.2.md)：実装済み（縮小版。[result/v0.2.md](../result/v0.2.md)）
- [v0.3 の詳細設計と PR 計画](../plan/v0.3.md)：未着手。次に着手する版の詳細設計
- [実装記録](../result/)：v0.1・v0.2 の実装と測定の記録
...
**最初の到達目標（2026-09-16 決定）**：**v0.6 の完了後、入力・出力のイベント駆動化を経て、最初の YouTube Live 配信を行う。** v1.0 の完了は待たない。これは開発の到達目標であって、設計原則の変更ではない——上段の「配信の有無にかかわらず、ローカルでの交流だけでも目的を達成できる設計とする」は維持する。

**v0.6 を配信の起点に置く理由**：配信中の自発的発話は v0.6 の候補選択を前提にする（第7節）。v0.5 以前で配信すると、コメントが途切れた時間に YUI が何も言えない——「YUI から話し始めることはない」を解くのが v0.5（会話の口火）と v0.6（会話中の発話機会の選択）だからである。配信は進行中の会話に相手が入ってくる場なので、口火だけでは足りない。

配信を目標に置くことで変わるのは次の3点である。

- 第7節の配信関連トラック（「YouTube 入力・応答」「配信中の自発的発話」）が、条件が揃い次第の独立トラックではなく **v0.6 完了後の必須項目**になる
- 第10節の「配信前は必須」の項目に**期限が付く**（v0.6 完了後・初配信前）
- **初配信は版ではなく、v0.6 と v1.0 の間に置く独立したマイルストーン**として扱う（第2.3節・第2.4節）
...
**v0.1 が実装済みで、実モデルで測定済み。v0.2（会話理解と修復）も実装済み
——ただし「縮小版」（第8.2節、[result/v0.2.md](../result/v0.2.md)）。**
範囲・検証済みの内容・既知の失敗は第8.1節（v0.1）・第8.2節（v0.2）。
v0.2 → v0.3 へ進む判断（§2.4）は 2026-09-15 に実施し、**縮小版のまま v0.3 へ進む**と決めた（判断の根拠と、v0.2 の残件を持ち込まないための取り決めは [result/v0.2.md](../result/v0.2.md) の「v0.2 → v0.3 の判断点」）。

| 数字 | 値 | 出所 |
| --- | --- | --- |
| backend の制御テスト | pytest 282件 | 2026-09-13、v0.2 PR5 |
| 評価シナリオ | 58本（機械判定52、人手のみ6） | `backend/evals/scenarios/` |
| 直近の回帰 | 38/52（解釈なし＝当時の既定、各1回。既定は 2026-09-16 に解釈ありへ変更した） | `logs/evals/20260913T165201-2026-09-07.1`、qwen3.5:9b、人格 `2026-09-07.1` |
| 解釈ありの回帰（`conversation_state` のみ） | 49/72（各3回） | `logs/evals/20260913T141036-2026-09-07.1`（`report.py` の `conversation_state_llm` 対応前の実行のため、解釈の有無はレポート単体では確認できない） |
...
| **v0.2**<br>会話理解と修復<br>（実装済み・縮小版） | 現在の用件、未回答の質問、提示済み・確認済みの内容、延期・終了の意思を会話の中で扱い、食い違いを断定せずに確かめる | 「そろそろ寝るね」→ 用件があっても新しい質問を出さずに締める。「後でね」と言った話題を同じ会話で振り直さない。「さっき土曜って言ったけど日曜だった」→ 確かめずに受け入れ、以後は日曜として扱う。訂正かどうか不明なときだけ1回確かめる | ⑥⑦ |
...
| **v1.0**<br>長期の統合評価 | 記憶・感情・関心・関係・行動が、長期の交流で一つの個体として機能することを確認する。**初配信の後の版**——ローカルと配信の両方で確認する | 数週間の交流で、着眼点・好み・関係の距離が経験の違いに応じて別々に残っている | 全項目 |

**初配信は版に数えない（2026-09-16）**：最初の YouTube Live 配信は **v0.6 の完了後・v1.0 の着手前**に置く（第1.1節）。上の索引に行を作らないのは、配信が「YUI が何をできるようになるか」の到達点ではなく、**できるようになったことを公開の場へ出す工程**だからである。判断点は第2.4節に置く。
...
| v0.2 → v0.3 | 終了・延期の意思を無視した発話が固定シナリオで 0件。修復の確認が過剰確認になっていない（第9節）。解釈を有効にした構成で測る | 縮小版（解釈なし）のまま進むかを別に判断し、記録する。v0.2 の残りを v0.3 に持ち込まない |
...
| v0.6 → **初配信** | 不適切発話率と不適切沈黙率の両方が v0.5 より悪化しない（版としての v0.6 の条件）。**配信の追加条件**（第1.1節）：(a) 入力・出力のイベント駆動化（第7節「YouTube 入力・応答」）が動く (b) 限定公開のテスト配信（**自発を切る**）で「配信で決めること」の未確定値を観測し、確定している (c) 第10節の配信前必須項目（自動採用のゲート・配信前の防御）と感度クラスが入っている | 揃うまで公開の場へ出さない。特に (b) を飛ばして自発的発話を公開配信へ出さない——**感度クラスが未確定のまま、私的な記憶から派生した話題が公開の場に出る** |
| 初配信 → v1.0 | 配信で新たに分かった問題（不適切発話、話題の遮り、公開範囲の漏れ）を v1.0 へ持ち込まない | 長期評価を始めない。**アーキテクチャは v1.0 着手前に凍結する**——評価期間中に構成を変えると前後の比較が成立しない |
...
**例外（2026-09-16）**：「YouTube 入力・応答」と「配信中の自発的発話」の2つは、第1.1節の最初の到達目標により **v0.6 完了後の必須項目**へ格上げした。条件が揃い次第ではなく、v0.6 → 初配信の判断点（第2.4節）までに完了させる。残る能力は従来どおり独立トラックとし、版の前提にしない。
...
| YouTube 入力・応答 | コメントを共通イベントへ変換し、同じ中核へ接続する。**宛先・公開範囲・発話順序を配信側で扱う**。重複排除、優先順位、期限、発話キュー。私的な記憶から派生した状態・目標にも公開範囲を引き継ぐ | v0.2 の会話状態と公開範囲（**充足済み**）。配信前に品質改善項目（第10節）の防御を入れる。**初配信の必須項目。期限は v0.6 完了後・初配信前** |
| 配信中の自発的発話 | **v0.6 の候補選択に、配信全体の話題・参加者・発話順序を接続する。** 視聴者の参加は候補を再評価する機会であり、即発話の根拠ではない。配信は進行中の会話に相手が入ってくる場なので、現在の話題を遮らないこと、個人的な話題を公開の場で蒸し返さないこと（感度クラス）、直近に発言した相手にだけ向けることを配信側で決める | v0.6。**初配信の必須項目。期限は v0.6 完了後・初配信前** |
...
**観測の機会（2026-09-16 追記）**：初配信を待つと、値が決まらないまま公開の場へ出ることになる。v0.6 完了とイベント駆動化の後、**初配信の前に**、**自発的発話を切った限定公開のテスト配信**（コメントの取り込みと返信のみ）を1回行い、そこで上の値を決める。この配信は入力・出力のイベント駆動化の検証を兼ねる。**自発を切るのは、未確定値（とりわけ感度クラス）が決まるまで、私的な記憶から派生した話題が公開の場へ自発的に出る経路を作らないため。** テスト配信は初配信の準備工程であり、初配信そのものではない。
...
**実装済みの範囲**（旧フェーズ1〜3の実装記録の要点を本節へ移し、`docs/result/phase1〜3.md` は削除した。測定値・数表の原文は git 履歴にある：`git log --diff-filter=D -- docs/result/phase1.md`）
...
**これは「必ず成功する保証」ではない。** v0.1 完了時点（2026-09-10、ログ `20260910T141904`）の回帰は 71/84 で、同一コードの2回実行で 43シナリオ中16が動く程度の揺れがある（[ISSUE-032](../issues/issues.md#issue-032)）。現在の数字は §2.1 を参照。
...
### 8.2 v0.2：会話理解と修復（実装済み・縮小版）

詳細設計と PR 計画は [v0.2.md](../plan/v0.2.md)。測定と判断の記録は
[result/v0.2.md](../result/v0.2.md)。**解釈（LLM）は 2026-09-16 に既定で有効に
した**（`conversation_state_llm = True`、timeout 30 秒）。一度は待ち時間と判定
精度を理由に見送ったが、精度を疑った3件はいずれも原因が別にあった——連続する
訂正は状態管理側（[ISSUE-044](../issues/issues.md#issue-044)）、答え損ねは
シナリオの前提と検出規則（[ISSUE-045](../issues/issues.md#issue-045)）、食い違いは
検出でなく `correction` との分類（[ISSUE-048](../issues/issues.md#issue-048)）。
詳細は [result/v0.2.md](../result/v0.2.md)「既定を有効に変更した」。

**ただし縮小版の記録は外さない。** 規則だけで満たせる範囲（下の成功条件
(1)(2)(5)(7)）は実装・測定済み。(3)(4)(6) は解釈が無いと到達できず、有効化に
よって初めて動き始めた段階で、(4) はまだ満たしていない
（[ISSUE-049](../issues/issues.md#issue-049)）。

| 項目 | 内容 |
| --- | --- |
| 新たにできること | 同じ会話の中で、**現在の用件**、**未回答の質問**（YUI が聞いて答えが無いもの、相手が聞いて YUI が答えていないもの）、**提示済み・確認済みの内容**、**延期・終了の意思**を扱う。相手の明示的な訂正はその会話で受け入れ、訂正かどうか不明な食い違いだけを断定せずに1回確かめる（修復）。**発言があったことと解決したことを分けて持つ**（一度聞いた未回答の質問を、繰り返さずに覚えておく）。完全な他者心理の推論は目指さない |
| 会話での見え方 | 「そろそろ寝るね」→ 用件があっても新しい質問を出さず、短く締める。「その話は後で」→ 同じ会話で振り直さない。相手の質問に答えないまま自分の話題へ移らない。「さっき土曜って言ったけど日曜だった」→ 確かめずに受け入れ、以後はこの会話では日曜として扱う（記憶は書き換えず、開発者へ訂正の候補として出す）。訂正かどうか不明なときだけ「日曜でしたか」と1回確かめ、返答が不明確なら再質問しない。「後でね」の話題を相手が自分から再開すれば応じる。求められていない説明は繰り返さず、再説明を求められたら言い換える |
| 再利用する既存機構 | 会話履歴（`messages`）、直近12件の履歴の注入、記憶の検索、`character_states` の関係性、再生状態の記録、`/end` の振り返り、評価器の say / reflect 手順 |
| 追加する仕組み | **会話状態**（会話ごとの用件・未回答・提示済み・確認済み・延期・終了・食い違い。根拠発言と解消の記録）。相手の発言の解釈（LLM の小さな呼び出し。失敗時は表層規則へ戻す）。プロンプトの「この会話で」の節。終了の合図の後に YUI から新しい用件を出さない出力検査（相手の最後の依頼には答える）。会話内で有効な訂正を記憶より優先して渡す。食い違いの確認は同じ対象につき1回まで（過剰確認の抑制）。状態は発言した相手と適用する相手を分けて持ち、別の相手の発言では更新しない |
| 前提 | v0.1。会話状態は会話の中で閉じ、長期記憶へは振り返り経由でのみ影響する |
| 対象外と既知の制限 | セッションをまたぐ共有理解の持続（v0.6 の関係の運用で扱う）。相手の感情の推定は「疲れている・話したくない」の表明に限り、内面の推論はしない。記憶の自動訂正はしない（訂正は会話の中でだけ有効。開発者へ候補として出す）。同意の記録はしない（理解の確認だけ）。検証は1対1に限る。解釈の LLM 呼び出しが増えるため待ち時間が伸びる（+77%〜+184%）。一度は既定を無効にしたが、2026-09-16 に有効へ変更した。[result/v0.2.md](../result/v0.2.md) |
| 成功条件 | (1) 終了の合図（「寝る」「また今度」等）の後に、新しい質問・用件を出した返答が固定シナリオで 0件 (2) 延期された話題を同じ会話で振り直した返答が 0件 (3) 相手の質問に答えていない状態で自分の話題へ移った返答が、対応する各シナリオで3回中2回以上（過半数）は起きない（PR5で実測して決めた閾値。[result/v0.2.md](../result/v0.2.md)） (4) 明示的な訂正は確かめずに受け入れて以後従い、不明な食い違いだけ**断定せず1回確認する**。同じ対象を再確認しない（過剰確認の抑制） (5) 一度伝えた説明の繰り返しが減る（`expect_not_repeating` で見る） (6) 状態が保存されただけでなく、(1)〜(5) の返答の違いとして現れる (7) 会話状態の更新が失敗しても返答は返る (8) v0.1 の回帰33本と pytest が通る |
| 失敗例 | 終了の合図を無視して切り出す／延期を忘れる／相手の質問を放置する（不適切な発話・沈黙）。食い違いを断定して押し切る／毎ターン確認する（過剰確認）。相づちだけで「理解された」と記録する。「発話した」を「理解された」と扱う。相手の次の発言が来ただけで質問や食い違いを解決済みにする。訂正を受け入れずに確かめ直す |
| 安定性 | 合否の正しさ（上の (1)〜(5)）と、試行間の安定性（3回とも同じ合否）を別に数える。3回とも失敗を「安定」と数えない |
| 完了の扱い | 成功条件は**解釈の LLM 呼び出しを有効にした構成で測る**。待ち時間の理由で無効を既定に選ぶ場合は、全到達目標を達成したとは扱わず「縮小版」として記録する（規則だけで満たせるのは (1)(2)(5)(7)） |
...
| 前提 | v0.2〜v0.6。簡略化した版があってもよい。**加えて初配信が完了していること**（第1.1節。第7節の配信2トラックと第10節の配信前必須項目は初配信の時点で入っている）。**アーキテクチャは v1.0 着手前に凍結する**——評価期間中に構成を変えると前後の比較が成立しない |
| 成功条件 | (1) 同じ YUI へ異なる経験を与え、会話後の変化を比較する。人格定義の効果と経験更新の効果を混同しない (2) 着眼点の違い、独立した好み、共有経験の経緯、質問と待機のバランス、身体的実体験の捏造の有無を人手で読む (3) 人格の核が全条件で保たれる (4) エンゲージメントや発話回数を完成度の代理にしない (5) ローカルの評価を多人数配信での自然さの保証にしない (6) **ローカルと配信の両方で確認する**（初配信は v1.0 の前に済んでいる。第1.1節）。配信側の成否は視聴者数・滞在時間・発話回数で測らない——不適切発話が出ないこと、現在の話題を遮らないこと、私的な記憶から派生した内容が公開の場へ出ないことを人手で読む |
...
**レイテンシ予算**（[ISSUE-054](../issues/issues.md#issue-054)、2026-09-17 決定）

「体験・安定性」の待ち時間は、何 ms までなら許容かを決めていなかった。実測の相対値（「解釈を有効にすると+77〜184%」）だけでは、最適化に**いつ着手し、いつ止めるか**を判断できない。

| 場面 | 目標値 | 根拠 |
| --- | --- | --- |
| ローカルの1対1テキスト会話（v0.1〜v0.3 の検証範囲）。ターン全体（解釈＋生成＋出力検査。再生成1回を含む）の壁時計時間 | **p50 ≤ 10秒、p95 ≤ 15秒**（**p50 は現状未達**。下記） | **達成目標であって、現状の追認ではない。** 2026-09-17 のターン全体の実測（[result/v0.2.md](../result/v0.2.md)「再測定」）は中央値 11.9秒・最大 14.85秒で、**p50 を超えている**。目標を実測に合わせて緩めない理由は、[ISSUE-058](../issues/issues.md#issue-058)（解釈の非同期化）で約7秒がクリティカルパスから外れれば達成できる見込みがあり、緩めると予算が最適化の圧力として働かなくなるため。快適さの確証ではなく、**縮めるべき量を示す値**である |
| 配信時（v0.6 完了後の初配信前に実測して確定。現時点は候補値） | p95 ≤ 8秒 | 視聴者との対話はローカルより短い遅延が期待される一方、複数視聴者・OBS 等との資源競合（§7）がある。実測前に確定させない |

**この予算の使い方**：

- `turn_time_budget_seconds`（既定45.0秒、`Settings`）は**別の値**として残す。これは「これを超えたら打ち切る」ハードリミットの候補で、まだ何も強制していない。上のp95予算は「通常はここに収める」という目標値で、両者を混同しない
- 予算を**継続的に**（3回の測定のうち複数回）超えたときに、[ISSUE-058](../issues/issues.md#issue-058)（解釈の非同期化）の着手を検討する。**単発の超過では着手しない**——3試行では効果と揺れを分離できないという評価の原則（下記）と同じ理由。2026-09-17 の p50 超過は1回目にあたるので、この規則によりまだ着手しない
- **測定は1回5ターンなので、p95 は推定できない**（5サンプルの最大はおよそ83パーセンタイル）。p95 を判定に使うなら、シナリオ数か会話の長さを増やしてから測る。現状は最大値を上端の目安として読む
- **ターンが進むほど遅くなる**（2026-09-17 の実測でターン4・5 が 14.8秒・14.4秒）。解釈に渡す開いている状態が会話とともに増えるためで、**5ターンより長い会話では p95 を超える**。原因は未特定
- v0.3 は感情の評価にLLM呼び出しを既定で追加しない（[v0.3 の計画](../plan/v0.3.md) §0）ため、この予算に新たな圧力を加えない。予算超過の主因は解釈（LLM）呼び出しのままである。**v0.3 の前後比較の基準値は 2026-09-17 の実測**（平均12.4秒）であり、v0.3 で12秒前後が出てもそれは v0.3 の増分ではない
...
| 再接続で同じコメントを再取得 | 二重応答・二重記憶・二重更新を起こさない | 初配信（テスト配信で確認する） |
...
**期限（2026-09-16）**：第1.1節で初配信を v0.6 完了後に置いたため、「配信前は必須」の項目に期限が付いた——**v0.6 完了後・初配信前**（第2.4節「v0.6 → 初配信」の条件 (c)）。対象は「自動採用のゲート」「配信前の防御」と、その前提になる「ラベルの収集」。このうち**ラベルの収集だけは、期限ではなく今から着手する**（理由は該当行）。
...
| ラベルの収集 | 手動採用は「まだ自動化していない状態」ではなく、**ラベル収集そのもの**。判断・日付・一言メモ・素性・抽出器の刻印（プロンプトのハッシュ・モデル名。**後から復元できないので初日から**）。5件に1件を評価用に回す（会話単位で分ける）。却下理由のコードはメモを30件読んでから決める。「知っているから採用」を禁じ、発言が支持しているかに対してラベルを付ける。**着手時期：v0.3 と並行して今から。** 刻印は後から復元できず、収集していない期間のラベルは永久に失われる。期限のある項目（上記）の中で、**前倒しが必要なのはこれだけ**——他は v0.6 完了後に着手しても費用が変わらない | Peduzzi 1996（1変数あたり少数クラス10件） |
...
| 関係の距離感の運用の形 | v0.6 |
| 感度クラス、発火の率と間隔、セッション境界、抽出の非同期化 | v0.6 完了後の限定公開テスト配信の観測後（第7節「観測の機会」） |
