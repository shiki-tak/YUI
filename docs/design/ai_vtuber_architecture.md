...
- 記憶・状態・目標は**すべて相手（`partner_id` / `subject_speaker_id`）に紐づく**。ローカルの1人会話でも配信でも同じ構造で持ち、違うのは相手が現れる形だけ
...
| 発火タイミング | **相手が現れたときのみ**。ローカルでは会話の開始、配信ではその相手がチャットに現れたとき。**v1.0 は前者だけを実装し、判定側（発火条件・目標・記憶）は相手の現れ方に依存しない形で作る**。配信では発火のトリガだけを差し替える（第14.1節 6B）。話題の区切りや会話中には発火しない。**相手の第一声を待たない**——発火した場合、セッションの最初の発言は YUI のものになる。これは条件ではなく前提で、相手の発言を見てから話すなら「相手の状態を見て黙る」経路が必要になり（第10.6節でやらないと決めたもの）、第10.3節も崩れる |
...
| 完了 | **`asked_at` を過ぎた後、同じセッション内で相手から1件でもメッセージが来たら `done`。`completed_at` はそのメッセージの時刻。** 返答内容の解釈はしない。したがって**「聞こえなかった」「後で」も done になる**。これは意図した割り切りで、区別は返答の中身でしかできず（[ISSUE-034](../issues/issues.md#issue-034)）、v1.0 では持たない。戻す先は v1.2 |
...
| 取消 | 元の記憶を削除したら **`adopted` の目標だけ** `dropped`（深さ1）。**`done` は触らない**——「すでに聞いた」は記憶が誤りでも消えない事実で、部分ユニークが `done` を含むのも再質問を防ぐためなので、落とすと制約が外れる。`dropped_reason`（`grounding_invalidated` / `rejected` / …）を残す。「根拠が消えて落ちた」と「方針で落とした」を区別できないと、同じ根拠が別の記憶から立ったときに重複判定ができない。**開発者の操作だけ。** 相手が会話で「やめた」と言っても v1.0 では目標が残る。会話からの取消は v1.1 の振り返りで戻す |
...
⑨ 記憶削除 → dropped の連動（深さ1の波及を作る）。**対象は adopted の目標だけ。**
...
| 移行は**溢れ処理**から（レビュー待ちが溜まったら高確信だけ自動採用）。ローカルでは溢れがほぼ起きず、量が増えるにつれて自動が主になる | 同じ機構で手動から自動へ移れる |
...
| 逐語索引（原文の検索経路） | L0 | 1.0（索引）→ 1.1（生成へ接続） | 論文（arXiv:2601.00821、単著・未追試）。根拠は非対称性の逆転のほう |
...
| 却下理由コード・hard gate の3値記録・抽出器の刻印 | 横断 | 1.0 | 設計案。抽出器の誤りとゲートの判断を分けて学ぶための前提 |
...
**発火のトリガだけを差し替える。** v1.x の目標・発火判定・記憶・採用ゲートは相手の数に依存しない形で作ってある（第1.2節、第10.1節）ので、配信で足すのは「その相手がチャットに現れた」をトリガにする部分と、コメントが少ない時の話題提案、コメントへの切り替え、必要なら配信前調査である。短い活動から時間を伸ばし、繰り返し・復旧・利用量を確認する。自発的に会話を進めることと、配信開始・終了の予約実行は別機能である。配信の自動開始を既定要件にはしない。
