...
| 直近の回帰 | 38/52（解釈なし・既定構成、各1回） | `logs/evals/20260913T165201-2026-09-07.1`、qwen3.5:9b、人格 `2026-09-07.1` |
...
[result/v0.2.md](../result/v0.2.md)。**解釈（LLM）は既定で無効のまま**——
待ち時間（+77%〜+184%、測定条件で変動）と実モデルでの判定精度（食い違いの検出・連続する訂正が弱い）
から、有効化を見送った。規則だけで満たせる範囲（下の成功条件 (1)(2)(5)(7)）
は実装・測定済みで、(3)(4)(6) は解釈が無いと到達できない。
...
| 対象外と既知の制限 | セッションをまたぐ共有理解の持続（v0.6 の関係の運用で扱う）。相手の感情の推定は「疲れている・話したくない」の表明に限り、内面の推論はしない。記憶の自動訂正はしない（訂正は会話の中でだけ有効。開発者へ候補として出す）。同意の記録はしない（理解の確認だけ）。検証は1対1に限る。解釈の LLM 呼び出しが増えるため待ち時間が伸びうる（測った結果、既定は無効のまま。[result/v0.2.md](../result/v0.2.md)） |
