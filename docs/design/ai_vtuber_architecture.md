...
| 場面 | 目標値 | 根拠 |
| --- | --- | --- |
| ローカルの1対1テキスト会話（v0.1〜v0.3 の検証範囲）。ターン全体（解釈＋生成＋出力検査。再生成1回を含む）の壁時計時間 | **p50 ≤ 10秒、p95 ≤ 15秒** | 解釈を既定で有効にした状態の実測（[result/v0.2.md](../result/v0.2.md)「既定を有効に変更した」）：解釈呼び出し単体で中央値 8.6〜9.0秒・最大 10.0〜12.0秒（timeout 30秒、2026-09-16）。生成時間（約4秒）を足すと合計は15秒に近づき、超えることもありうる。**現状ぎりぎり収まる値として置き、実測を継続する**候補値であって、快適さの確証ではない |
...
- 予算を**継続的に**（3回の測定のうち複数回）超えたときに、[ISSUE-058](../issues/issues.md#issue-058)（解釈の非同期化）の着手を検討する。**単発の超過では着手しない**——3試行では効果と揺れを分離できないという評価の原則（下記）と同じ理由
- v0.3 は感情の評価にLLM呼び出しを既定で追加しない（[v0.3 の計画](../plan/v0.3.md) §0）ため、この予算に新たな圧力を加えない。予算超過の主因は解釈（LLM）呼び出しのままである
