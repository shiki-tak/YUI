...
**最初の到達目標（2026-09-16 決定）**：**v1.0 の完了時点で、YouTube Live 配信を1回完了できる状態にする。** これは開発の到達目標であって、設計原則の変更ではない——上段の「配信の有無にかかわらず、ローカルでの交流だけでも目的を達成できる設計とする」は維持する。配信を目標に置くことで変わるのは次の2点である。

- 第7節の配信関連トラック（「YouTube 入力・応答」「配信中の自発的発話」）が、条件が揃い次第の独立トラックではなく **v1.0 の必須項目**になる
- 第10節の「配信前は必須」の項目に**期限が付く**（v0.6 完了後・v1.0 着手前）

いずれも v0.6 → v1.0 の判断点（第2.4節）で確認する。
...
| **v1.0**<br>長期の統合評価<br>＋ YouTube Live | 記憶・感情・関心・関係・行動が、長期の交流で一つの個体として機能することを確認する。**完了時点で YouTube Live 配信を1回完了する**（第1.1節の最初の到達目標） | 数週間の交流で、着眼点・好み・関係の距離が経験の違いに応じて別々に残っている。配信では、視聴者のコメントに応じながら、現在の話題を遮らずに自分から話す | 全項目 |
...
| v0.6 → v1.0 | 不適切発話率と不適切沈黙率の両方が v0.5 より悪化しない。**配信を到達目標に置いたことによる追加条件**（第1.1節）：(a) 入力・出力のイベント化（第7節「YouTube 入力・応答」）が動く (b) 限定公開のテスト配信で「配信で決めること」の未確定値を観測し、確定している (c) 第10節の配信前必須項目（自動採用のゲート・配信前の防御）が入っている | 片方だけの改善で進まない。(a)〜(c) が揃わない場合は **v1.0 の長期評価を始めない**——評価期間中に構成を変えると前後の比較が成立しないため、アーキテクチャは v1.0 着手前に凍結する |
...
**例外（2026-09-16）**：「YouTube 入力・応答」と「配信中の自発的発話」の2つは、第1.1節の最初の到達目標により **v1.0 の必須項目**へ格上げした。条件が揃い次第ではなく、v0.6 → v1.0 の判断点（第2.4節）までに完了させる。残る能力は従来どおり独立トラックとし、版の前提にしない。
...
| YouTube 入力・応答 | コメントを共通イベントへ変換し、同じ中核へ接続する。**宛先・公開範囲・発話順序を配信側で扱う**。重複排除、優先順位、期限、発話キュー。私的な記憶から派生した状態・目標にも公開範囲を引き継ぐ | v0.2 の会話状態と公開範囲（**充足済み**）。配信前に品質改善項目（第10節）の防御を入れる。**v1.0 必須。期限は v0.6 完了後・v1.0 着手前** |
| 配信中の自発的発話 | **v0.6 の候補選択に、配信全体の話題・参加者・発話順序を接続する。** 視聴者の参加は候補を再評価する機会であり、即発話の根拠ではない。配信は進行中の会話に相手が入ってくる場なので、現在の話題を遮らないこと、個人的な話題を公開の場で蒸し返さないこと（感度クラス）、直近に発言した相手にだけ向けることを配信側で決める | v0.6。**v1.0 必須。期限は v0.6 完了後・v1.0 着手前** |
...
**観測の機会（2026-09-16 追記）**：本番配信を待つと、値が決まらないまま v1.0 に入る。v0.6 完了後、**自発的発話を切った限定公開のテスト配信**（コメントの取り込みと返信のみ）を1回行い、そこで上の値を決める。この配信は入力・出力のイベント化の検証を兼ねる。**自発を切るのは、未確定値が決まるまで公開の場へ自発的発話を出さないため。**
...
| 前提 | v0.2〜v0.6。簡略化した版があってもよい。**加えて、第7節の配信2トラックと第10節の配信前必須項目が完了していること**（第2.4節の判断点）。**アーキテクチャは v1.0 着手前に凍結する**——評価期間中に構成を変えると前後の比較が成立しない |
| 成功条件 | (1) 同じ YUI へ異なる経験を与え、会話後の変化を比較する。人格定義の効果と経験更新の効果を混同しない (2) 着眼点の違い、独立した好み、共有経験の経緯、質問と待機のバランス、身体的実体験の捏造の有無を人手で読む (3) 人格の核が全条件で保たれる (4) エンゲージメントや発話回数を完成度の代理にしない (5) ローカルの評価を多人数配信での自然さの保証にしない (6) **YouTube Live 配信を1回完了する**（第1.1節）。配信の成否は視聴者数・滞在時間・発話回数で測らない——不適切発話が出ないこと、現在の話題を遮らないこと、私的な記憶から派生した内容が公開の場へ出ないことを人手で読む |
...
| 再接続で同じコメントを再取得 | 二重応答・二重記憶・二重更新を起こさない | v1.0（配信。テスト配信で確認する） |
...
**期限（2026-09-16）**：第1.1節で配信を v1.0 の到達目標に置いたため、「配信前は必須」の項目に期限が付いた——**v0.6 完了後・v1.0 着手前**（第2.4節の判断点 (c)）。対象は「自動採用のゲート」「配信前の防御」と、その前提になる「ラベルの収集」。このうち**ラベルの収集だけは、期限ではなく今から着手する**（理由は該当行）。
