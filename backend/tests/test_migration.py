...
def test_migration_backfills_visibility_scope(tmp_path, monkeypatch):
    ...
    with sqlite3.connect(db_path) as con:
        ...
        candidates = dict(con.execute("select id, visible_to_speaker_id from memory_candidates"))
        reflections = dict(
            con.execute("select id, reflection_completed_at is not null from conversations")
        )
